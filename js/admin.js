...
// ✅ When selecting user, override default auth flow
userSelector.addEventListener("change", async () => {
  ...
  const fakeUser = { uid: selectedUserId };

  await updateUserInventoryUI(fakeUser);
  await fetchEQScoreProgressFake(fakeUser);
  await fetchEQMissedFrequenciesTableFake(fakeUser);

  // 🔥 NEW — fire admin metrics
  await fetchMissedFrequenciesFake(selectedUserId, today, timeFilter);
  await fetchScoreProgressFake(selectedUserId);
});
...
async function fetchEQScoreProgressFake(user) {
...
async function fetchEQMissedFrequenciesTableFake(user) {
  const userId = user.uid;
  if (!userId) return;
...
// ✅ Hook on page load or auth success
onAuthStateChanged(auth, async (user) => {
  if (user) {
    // Admin flow
