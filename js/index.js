...
// ==================== PLAN STORAGE HANDSHAKE ==================== //
(function ensurePlanPersistence() {
  // Se existir plano ativo e não houver cópia direta, recria
  const key = localStorage.getItem("currentPlanKey");
  if (key && !localStorage.getItem("currentPlan")) {
    const saved = localStorage.getItem(key);
    if (saved) {
      localStorage.setItem("currentPlan", saved);
      console.log("🔄 Recriada cópia direta de plano ativo:", key);
    }
  }
})();
...
// data charts SineWave /////////

let missedFrequenciesChart;

let scoreProgressChart;

    const today = new Date().toISOString().split("T")[0];
    document.getElementById("date-picker-missed").value = today;
    document.getElementById("time-filter-missed").value = "week";

    auth.onAuthStateChanged((user) => {
        if (user) {
            fetchMissedFrequencies(); // ✅ Run only after login
        } else {
...
document.getElementById("export-missed-chart").addEventListener("click", () => {
    ...
    const fileName = `SineWaveTrainer_MissedFrequencies_${username}_${timestamp}.png`;
...
// modal popup on chart click

document.getElementById("missedFrequenciesChart").addEventListener("click", () => {
    if (!missedFrequenciesChart) {
        alert("Chart not loaded yet!");
        return;
    }
    ...
    if (window.modalMissedChart) {
        window.modalMissedChart.destroy();
    }

    window.modalMissedChart = new Chart(modalCtx, {
        type: missedFrequenciesChart.config.type,
        data: JSON.parse(JSON.stringify(missedFrequenciesChart.config.data)),
        options: missedFrequenciesChart.config.options
    });
});
...
// Sine Wave Trainer Score Progress Modal and export .png

// ✅ Export PNG with metadata in filename
document.getElementById("export-score-chart").addEventListener("click", () => {
    ...
    const fileName = `SineWaveTrainer_ScoreProgress_${username}_${timestamp}.png`;
...
// ✅ Modal popup for fullscreen chart
document.getElementById("scoreProgressChart").addEventListener("click", () => {
    if (!scoreProgressChart) {
        alert("Chart not loaded yet!");
        return;
    }
    document.getElementById("score-chart-modal").style.display = "flex";

    const modalCtx = document.getElementById("scoreModalChart").getContext("2d");
    if (window.modalScoreChart) {
        window.modalScoreChart.destroy();
    }

    window.modalScoreChart = new Chart(modalCtx, {
        type: scoreProgressChart.config.type,
        data: JSON.parse(JSON.stringify(scoreProgressChart.config.data)),
        options: scoreProgressChart.config.options
    });
});

document.getElementById("close-score-modal").addEventListener("click", () => {
    document.getElementById("score-chart-modal").style.display = "none";
    if (window.modalScoreChart) {
        window.modalScoreChart.destroy();
    }
});
...
document.getElementById("fetch-missed-data").addEventListener("click", fetchMissedFrequencies);

// MISSED FREQUENCIES FUNCTION

async function fetchMissedFrequencies() {
    try {
        const user = auth.currentUser;
        if (!user) return console.warn("⚠️ No user logged in!");

        const selectedDate = document.getElementById("date-picker-missed").value;
        const timeFilter = document.getElementById("time-filter-missed").value;
        ...
        const selectedTimestamp = new Date(selectedDate + "T00:00:00");;
        ...
        console.log(`🔍 Fetching missed frequencies from: ${startTimestamp} to ${endTimestamp}`);
        ...
        console.error("🔥 Error fetching missed frequencies:", error);
    }
}
...
function updateMissedFrequenciesChart(labels, data) {
    ...
    if (missedFrequenciesChart) {
        missedFrequenciesChart.destroy(); // ✅ Prevents infinite memory growth
    }
    ...
    missedFrequenciesChart = new Chart(ctx, {
        ...
        options: {
            responsive: true, 
            maintainAspectRatio: false, // ✅ Ensures correct sizing
            scales: {
                y: {
                    beginAtZero: true
                }
            ...
            layout: {
                padding: 10 // ✅ Prevents elements from touching edges
            }
        }
    });
}
...
// ✅ Fetch and display score progress
async function fetchScoreProgress() {
    try {
        const user = auth.currentUser;
        if (!user) {
            console.warn("⚠️ No user logged in!");
...
function updateScoreProgressChart(labels, scores) {
    ...
    if (scoreProgressChart) {
        scoreProgressChart.destroy(); // ✅ Prevent memory leaks
    }
    ...
    scoreProgressChart = new Chart(ctx, {
        ...
        options: {
            responsive: true,
            maintainAspectRatio: false, // ✅ Prevents infinite growth
            scales: {
                y: {
                    beginAtZero: true,
                    max: 100 // ✅ Scores are percentages (0-100)
                }
            },
            animation: {
...
try {
 ...
 activityList.innerHTML = ""; //  Limpa updates prévios

 let activities = [];

 // cria um array das fetch promises da sessão
...
async function loadDrillPlan() {
  // 🔸 Primeiro tenta ler plano ativo diretamente (mais rápido)
  const currentPlan = localStorage.getItem("currentPlan");
  if (currentPlan) {
    console.log("📘 Usando plano ativo do localStorage.");
    return JSON.parse(currentPlan);
  }

  // 🔸 Senão, tenta ler via chave de plano ativo
  const selectedKey = localStorage.getItem("currentPlanKey");
  if (selectedKey && localStorage.getItem(selectedKey)) {
    console.log("📗 Usando plano salvo por chave:", selectedKey);
    return JSON.parse(localStorage.getItem(selectedKey));
  }

  // 🔸 Caso contrário, cai no default
  console.log("📙 Nenhum plano salvo encontrado. Usando plano padrão.");
  const res = await fetch("/json/semana-drills.json");
  return await res.json();
}
...
function renderVisibleDrills() {
  ...
  allDrills.slice(0, visibleDrillCount).forEach(drill => {
    ...
    // Nova, levando em consideração  Frequency Resolution

    const eqMatches = eqSessions.filter(s => {
    const st = s.settings || {};

    if (drill.EQFitnessPresetUsed !== "None") {
      // Caso normal: continua usando preset
      return st.presetUsed === String(drill.EQFitnessPresetUsed) &&
            st.playbackMode === drill.EQFitnessPlaybackMode &&
            st.gainCombination === drill.EQFitnessGainCombination;
    } else {
      // Caso sem preset: usamos frequencyResolution
      return st.frequencyResolution === drill.frequencyResolution &&
            st.playbackMode === drill.EQFitnessPlaybackMode &&
            st.gainCombination === drill.EQFitnessGainCombination;
    }
  });
  ...
  toggleBtn.textContent = visibleDrillCount >= allDrills.length ? "Fechar semanas" : "Mostrar mais";
}
...
async function updateUserInventoryUI(user) {
  ...
  // Coletar estatísticas para cada drill
  const drillStats = {};
